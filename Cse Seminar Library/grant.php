...
<div id="content">
	...
	<div id="colTwo"> <h2>Grant Book</h2>
    <p>
    <form action="grant.php" method="post">
                Book Id: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <input type="text" name="title" /> <br/><br/>
                Registration no: <input type="text" name="reg" />
                <input type="hidden" name="grantformsubmitted" value="TRUE" />
                <input type="submit" align="right" value="Grant" />
     ...
     <?php 
		include ('database_connection.php');                  
		if (isset($_POST['grantformsubmitted']))
		{
			$t=$_POST['title'];
			$r=$_POST['reg'];
			$grant_d=date("d.m.y");
			
			$return_d = mktime(0,0,0,date("m"),date("d")+7,date("Y"));
            $parsed_d=  date("d.m.y", $return_d);
			
			$query_insert_borrower =  "INSERT INTO borrower ( Title, Reg, Borrow_date, Return_date) VALUES ( '$t', '$r', '$grant_d', '$parsed_d')";
            $result_insert_borrower = mysqli_query($dbc, $query_insert_borrower);
			
			if (!$result_insert_borrower) {
                echo 'Query for inserting into borrower Failed ';
            }
			...
			if (mysqli_affected_rows($dbc) == 1) 
			{
				echo "Book is successfully granted.";
				}
